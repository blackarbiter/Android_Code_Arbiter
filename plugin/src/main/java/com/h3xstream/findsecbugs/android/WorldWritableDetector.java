...
//        printOpCode(seen);
        ...
        if (seen == Constants.INVOKEVIRTUAL &&
                (getNameConstantOperand().equals("getSharedPreferences")
                || getNameConstantOperand().equals("openFileOutput"))) {
...
//                System.out.println(value);
        ...
        }else if(seen == Constants.INVOKEVIRTUAL &&
                (getNameConstantOperand().equals("openOrCreateDatabase"))){
            OpcodeStack.Item item = stack.getStackItem(1); //second item on the stack is the last
            if(StackUtils.isConstantInteger(item)) {
                Integer value = (Integer) item.getConstant();
                if(value == null || value != 0) {
                    bugReporter.reportBug(new BugInstance(this, ANDROID_WORLD_WRITABLE_TYPE, Priorities.NORMAL_PRIORITY) //
